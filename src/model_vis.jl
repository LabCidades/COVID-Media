...
df = reduce(vcat, DataFrame(gen[:, i]) for i ∈ 1:size(gen, 2))
select!(df, Not([:dependent, :indirect, :direct, :total,
                 :τ_med, :τ_dep, :σ_dep, :σ_med]))
...
xticks = ["α_beh", "α_beh_tv", "α_beh_np", "α_beh_sm",
          "α_risk", "α_risk_tv", "α_risk_np", "α_risk_sm",
          "β_age", "β_sex_male", "β_selfeff",
          "β_media_beh", "β_media_risk", "β_risk_beh"
          ]
...
plt_violin = data(long_df) *
                  mapping(:parameter,
                          ...
                          # color=:model,
                          # dodge=:model) *
                  visual(Violin; show_outliers=false,
                         show_median=true,
                         width=0.95)
